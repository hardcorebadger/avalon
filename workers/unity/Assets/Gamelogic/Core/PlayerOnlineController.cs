	...
	public class PlayerOnlineController : MonoBehaviour {
		...
		public void OnContructionCreated(EntityId id, ConstructionRequest req) {
			...
			if (req.district.HasValue) {
				SpatialOS.Commands.SendCommand (playerOnlineWriter, District.Commands.RegisterBuilding.Descriptor, new BuildingRegistrationRequest (id, req.position, 0, new List<int>()), req.district.Value);
			}
		}
