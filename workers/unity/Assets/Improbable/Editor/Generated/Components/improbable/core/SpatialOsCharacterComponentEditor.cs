...
{
    [CustomEditor(typeof(SpatialOsCharacterComponent))]
    public class SpatialOsCharacterComponentEditor : UnityEditor.Editor
    {
        private SpatialOsCharacterComponent editedComponent;
        private SpatialOsCharacterComponentEditorData componentData;

        private class SpatialOsCharacterComponentEditorData : SpatialOsComponentEditorBase<SpatialOsCharacterComponent>
        {
            public int PlayerId
            {
                get { return Component.PlayerId; }
            }
        }

        protected virtual void OnEnable()
        {
            editedComponent = (SpatialOsCharacterComponent) target;

            var hashCode = editedComponent.GetHashCode();
            componentData = (SpatialOsCharacterComponentEditorData) EditorObjectStateManager.GetComponentEditorData(hashCode);
            if(componentData == null)
            {
                componentData = new SpatialOsCharacterComponentEditorData();
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.Space();
            EditorGUILayout.IntField("playerId", componentData.PlayerId);
