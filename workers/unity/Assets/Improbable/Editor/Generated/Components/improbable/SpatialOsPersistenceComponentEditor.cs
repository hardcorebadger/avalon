...
{
    [CustomEditor(typeof(SpatialOsPersistenceComponent))]
    public class SpatialOsPersistenceComponentEditor : UnityEditor.Editor
    {
        private SpatialOsPersistenceComponent editedComponent;
        private SpatialOsPersistenceComponentEditorData componentData;

        private class SpatialOsPersistenceComponentEditorData : SpatialOsComponentEditorBase<SpatialOsPersistenceComponent>
        {        }

        protected virtual void OnEnable()
        {
            editedComponent = (SpatialOsPersistenceComponent) target;

            var hashCode = editedComponent.GetHashCode();
            componentData = (SpatialOsPersistenceComponentEditorData) EditorObjectStateManager.GetComponentEditorData(hashCode);
            if(componentData == null)
            {
                componentData = new SpatialOsPersistenceComponentEditorData();
