...
namespace Improbable.Unity.Core
{
        ...
        IAuthorityChangedReceiver
    {
        ...
        {
            entityInterestedComponentsUpdater = new EntityComponentInterestOverridesUpdater(hostBehaviour);
        ...
        /// <inheritdoc />
        public void AuthorityChanged(EntityId entityId, IComponentMetaclass componentId, bool hasAuthority, object component)
        {
            spatialCommunicator.TriggerComponentAuthorityChanged(entityId, componentId, hasAuthority, component);
            for (int i = 0; i < authorityChangedCallbacks.Count; ++i)
            {
				try
				{
	                authorityChangedCallbacks[i](entityId, componentId, hasAuthority, component);
				}
				catch(Exception e)
				{
