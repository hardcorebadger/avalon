...
namespace Improbable.Assets
{
    ...
    /// /// </summary>
    /// <typeparam name="T">the type of resource to be stored and retrieved from the cache</typeparam>
    public class MachineCache<T> : MachineCache<T, T>
    {
        public MachineCache(string cachePath, IPersistenceStrategy persistenceStrategy) : base(cachePath, persistenceStrategy) { }
    }
    ...
    /// </remarks>
    public class MachineCache<TIn, TOut>
    {
        ...
        public bool TryAddOrUpdate(string key, TIn cacheItem)
        {
            ...
            {
                Debug.LogErrorFormat("Attempt to add or update an item to the cache failed.", e);
                return false;
            }
        }
        ...
        private static string GenerateUniqueToken(string caseInsensitiveKey)
        {
            ...
            return Convert.ToBase64String(new SHA256CryptoServiceProvider().ComputeHash(pathBytes)).ToUpperInvariant();
        }
        ...
        private static void TryAct(Func<bool> action, string path)
        {
            try
            {
                ...
                var mutexName = "Global\\" + GenerateUniqueToken(Path.GetFullPath(path) ?? path);
