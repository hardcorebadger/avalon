...
namespace Improbable.Unity.Core
{
    ...
    /// </summary>
    [InternalLogic]
    public class LegacyComponentPipeline : PassOpsBlock
    {
        ...
        public override void AddComponent(AddComponentPipelineOp addComponentOp)
        {
            ...
            if (entity != null)
            {
                ...
                {
                    ((EntityVisualizers)entity.Visualizers).OnComponentAdded(addComponentOp.EntityId, addComponentOp.ComponentMetaClass, addComponentOp.ComponentObject);
                }
            }
            else
            {
        ...
        public override void RemoveComponent(RemoveComponentPipelineOp removeComponentOp)
        {
            ...
            if (entity != null)
            {
                if (entity.Visualizers is EntityVisualizers)
                {
                    ((EntityVisualizers)entity.Visualizers).OnComponentRemoved(removeComponentOp.EntityId, removeComponentOp.ComponentMetaClass, removeComponentOp.ComponentObject);
                }
            }
            else
            {
