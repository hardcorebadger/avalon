...
using Improbable.Unity.Core.Internal;
...
namespace Improbable.Unity.Core
{
    ...
    /// </summary>
    [InternalLogic]
    public class LegacyEntityCreator : IEntityPipelineBlock, IDisposable
    {
        ...
        private readonly ulong criticalSectionCallbackKey;
        ...
        private void MakeEntity(EntityId entityId, string prefabName, Action<EntityId, string> onSuccess)
        {
            ...
            templateProvider.PrepareTemplate(prefabName,
                ...
                exception =>
                {
                    Debug.LogError("Exception occurred when preparing " + prefabName + " template. " + exception);
                });
        }
        ...
        private void DestroyEntity(IEntityObject entity)
        {
            ...
            entityComponentInterestOverridesUpdater.RemoveEntity(entity.EntityId);

            var entityObjectStorage = entity.UnderlyingGameObject.GetComponent<EntityObjectStorage>();
        ...
        private void OnMakeEntitySuccess(EntityId entityId, string prefabName)
        {
            if (entitiesToSpawn.ContainsKey(entityId))
            {
                ...
                universe.AddEntity(entityId, entityObject);
        ...
        public void ChangeAuthority(ChangeAuthorityPipelineOp changeAuthorityOp)
        {
            ...
            {
                Debug.LogErrorFormat("Entity not found: OnAuthorityChanged({0}, {1}) has authority: {2}",
                    changeAuthorityOp.EntityId, changeAuthorityOp.ComponentMetaClass, changeAuthorityOp.HasAuthority);
                return;
            }
        ...
        {
            var toDestroy = new System.Collections.Generic.List<IEntityObject>();

#pragma warning disable 0618
            universe.IterateOverAllEntityObjects((id, obj) => { toDestroy.Add(obj); });
#pragma warning restore 0618
            for (var i = 0; i < toDestroy.Count; i++)
            {
                var entityObject = toDestroy[i];
